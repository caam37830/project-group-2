...
def plotphasediagram(blist,klist,N,T,start):
    ...
    plt.colorbar()
    plt.title("Percentage of Population Infected and Removed T = "+str(T)+", Discrete")
...
def plotphasediagraminfect(blist,klist,N,T,start):
    ...
    plt.figure(figsize=(8,10))
    plt.imshow(cts,extent=[np.min(klist),np.max(klist),np.min(blist),np.max(blist)],interpolation="nearest",aspect="auto")
    plt.title("Percentage of Population Infected T= "+str(T)+", Discrete")
    plt.colorbar()
    plt.xlabel("change of k")
    plt.ylabel("change of b")
    plt.show()
...
def plotodephasediagram(x,blist,xlist,T):
    ...
    plt.imshow(cts,extent=[np.min(klist),np.max(klist),np.min(blist),np.max(blist)],interpolation="nearest",aspect='auto')
    plt.colorbar()
    plt.title("Percentage of Population Removed and Infected T = "+str(T)+", Continuous")
    plt.xlabel("change of k")
    plt.ylabel("change of b")
    plt.show()
...
def plotodeinfectphasediagram(x,blist,xlist,T):
    ...
    plt.imshow(cts,extent=[np.min(klist),np.max(klist),np.min(blist),np.max(blist)],interpolation="nearest",aspect='auto')
    plt.colorbar()
    plt.title("Percentage of Population Infected T = "+str(T)+", Continuous")
    plt.xlabel("change of k")
    plt.ylabel("change of b")
    plt.show()
